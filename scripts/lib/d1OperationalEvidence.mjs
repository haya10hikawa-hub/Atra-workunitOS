...
import { createHash, randomBytes, createPublicKey, createPrivateKey, sign as edSign, verify as edVerify } from "node:crypto"
...
// ─── Session signatures (Ed25519) ─────────────────────────────────
//
// Receipts are signed with a session-scoped Ed25519 key generated by the evidence
// initializer. The signature proves that a receipt came through ONE initialized
// repository evidence session — it is NOT a third-party Cloudflare attestation and
// does not protect against a malicious machine owner. The public key travels as
// the raw 32-byte key in lowercase hex; the private key exists only as a 0600 PEM
// inside the git-ignored session directory and never enters a record or a log.
...
/** Sign a receipt digest (utf8 of the 64-hex digest) with the session private key PEM. */
export function signReceiptDigest(privateKeyPem, receiptSha256) {
  const key = createPrivateKey(privateKeyPem)
  return edSign(null, Buffer.from(receiptSha256, "utf8"), key).toString("hex")
}
