...
test("1 + 2 + 3 + 4. migrations:apply refuses a same-id config before Wrangler — no ledger, no Control or Tenant migration", () => {
  ...
  assert.match(src, /execRemoteSqlText\(binding, authority, CREATE_HISTORY_SQL, "ledger-init"\)/,
    "the ledger is created by a remote write, through a scoped execution config from the retained authority")
