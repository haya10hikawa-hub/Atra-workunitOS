...
test("port modules import only ports and domain, and the port layer has no outbound edge", async () => {
  // Non-vacuity: an empty edge set proves nothing unless the scan root actually contains code.
  const portFiles = await collectCodeFiles(path.join(rootDir, "app/lib/ports"))
  assert.ok(portFiles.length >= 2, "app/lib/ports must contain the scanned contract modules")
  await assertNoForbiddenImports(["app/lib/ports"], "ports", violatesPortTarget)

  // The layer's zero-edge invariant, restored. It briefly carried one reviewed port-to-port type
  // edge, from the acquisition-evidence contract to the signal port; the revised contract depends
  // on no Atra projection, so that exception lost its subject and was retired with it. The scan
  // stays whole-layer: narrowing it to one module would stop observing every other port module,
  // which is precisely the closed-world property that makes an exact edge set meaningful.
  const portEdges = await scanModuleGraph(rootDir, ["app/lib/ports"])
  assert.deepEqual(portEdges.map(exactEdgeKey).sort(), [],
    "app/lib/ports must have no outbound edge at all")

  // Stated per module as well as layer-wide, so each keeps failing on its own terms if the other is
  // ever re-cut: both contract modules are graph leaves, and the dependency runs neither way.
  for (const leaf of [SIGNAL_PORT, EVIDENCE_PORT]) {
    assert.deepEqual(portEdges.filter((edge) => edge.file === leaf), [],
      `${leaf} must import nothing at all and stay a graph leaf`)
  }
