...
- **C** an existing destination is never overwritten, and is refused *before* any
  Gmail request rather than after it; a destination outside the authorized root —
  including a path-traversal attempt, a symlink directory planted inside the root
  that resolves outside it, and a symlink at the destination's own final path
  component — is refused before any fetch. `writeBytesDurable`'s `wx` flag keeps its
  own direct coverage as the last-resort race guard behind that pre-check
...
- **G** the pre-T0 preflight is content-free PASS/FAIL over exactly seven
  current-run structural checks, and takes no prior-run input at all
- **H** the Run-3 integration repairs: `readback_works` fails independently of
  `destination_writable`; no CLI command leaks a credential, subject, sender,
  recipient, marker or payload fragment on stdout *or* stderr — including on the
  success path, which is exercised as a real subprocess with an injected fetch stub

38/38 pass, and they pass with the network denied — no test reaches a real Gmail
host. `byteEqual` is a direct binary comparison of provider vs. persisted bytes, not a
digest comparison — B4 proves this with a forced digest collision. `assertInsideRoot`
resolves both the root and the destination's parent through `realpathSync`, and
`lstat`s the final component, before any fetch — so neither a symlinked directory nor
a symlinked basename can pass containment while resolving outside the root (C4, H-D1).

Eleven mutations (M1–M11) were replayed to confirm the suite exercises these
properties rather than passing vacuously; all eleven are detected, with no unexplained
survivors. Two of them are worth recording, because they found real gaps rather than
confirming existing coverage:

- **M8** (`wx` → `w`) initially survived. Adding the pre-fetch destination check meant
  every existing overwrite test stopped short of the `wx` flag, leaving the race guard
  uncovered. H-C2 now exercises `writeBytesDurable` directly.
- **M6** (token appended to acquire output) initially survived. Every CLI test drove a
  *failing* invocation, so the success-path output was never observed from a
  subprocess. H-B3 now runs the real CLI to a successful acquire with an injected
  fetch stub.
